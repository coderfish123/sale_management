<template>
  <div>
    ...
    <el-dialog title="商品信息" :visible.sync="fromVisible" width="40%" :close-on-click-modal="false" destroy-on-close @close="cancel">
...
export default {
  ...
  methods: {
    ...
    handleEdit(row) {   // 编辑数据
      ...
      initWangEditor(this.form.description || '')//传输descrip
      this.fromVisible = true   // 打开弹窗
    },
    viewEditor(content) {
      //传输内容
    ...
    save() {   // 保存按钮触发的逻辑  它会触发新增或者更新
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          this.form.description = editor.txt.html()//取出数据
          this.$request({
