<template>
  <div>
    <div style="display:flex;width: 70%;background-color:#fff;margin: 30px auto;border-radius: 20px">
...
export default {
  ...
  data() {
    let typeId = this.$route.query.id//从路由拿到分类的id
    return {
  ...
  methods:{
    loadGoods(){
      this.$request.get('/goods/selectByTypeId?id='+this.typeId).then(res=>{
        if (res.code === '200'){
          this.goodsData = res.data//返回的数据res.data赋值给goodsData
        }else{
